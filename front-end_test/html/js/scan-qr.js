...
function startScanning() {
    ...
    html5Qrcode.start(
        ...
        qrCodeMessage => {
            ...
            // QR 코드 데이터 처리 후 페이지 이동
            const encodedMessage = encodeURIComponent(qrCodeMessage);
            window.location.href = `./display-medication.html?data=${encodedMessage}`;
        },
        errorMessage => {
            console.error(errorMessage);
        }
    ).catch(err => {
        console.error('QR 코드 스캔 시작에 실패했습니다: ', err);
    });
}
//
//
